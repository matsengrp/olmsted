...
* `bin/build_datasets_manifest.js` rewrites `datasets.json` from scratch on each run (no more preserved split-format entries). Dropped `bin/aws_explore.py` and `bin/snapshot_server_data.py`, both of which only made sense in the legacy flow.
