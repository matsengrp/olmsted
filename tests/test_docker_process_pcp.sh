...
for img in "${SELECTED_IMAGES[@]}"; do
    ...
    sudo docker run --rm \
        ...
        "$img" \
        python bin/process_pcp_data.py -i /data/pcp/ford-flairr-seq-prod-UnmutInv_igh_pcp_2024-07-26.csv.gz -o /output \
        -v
...
# Final results
echo "🎯 PCP Processing Test Results:"
echo ""
all_success=1
for img in "${SELECTED_IMAGES[@]}"; do
    if [ "${IMAGE_SUCCESS[$img]}" -eq 1 ] && [ "${IMAGE_VALID[$img]}" -eq 1 ]; then
        ...
        if [ "${IMAGE_SUCCESS[$img]}" -eq 0 ]; then
            echo "     Debug with: sudo docker run --rm -v \$(pwd)/example_data:/data $img python bin/process_pcp_data.py -i /data/pcp/ford-flairr-seq-prod-UnmutInv_igh_pcp_2024-07-26.csv.gz -o /output -v"
        fi
    fi
done
...
if [ $all_success -eq 1 ]; then
    echo "🎉 SUCCESS: All PCP processing tests passed!"
    echo "✅ PCP data successfully converts to AIRR format"
else
    echo "❌ FAILURE: Some PCP processing tests failed"
fi