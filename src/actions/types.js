export const BRUSH_SELECT_CLONAL_FAMILIES = "BRUSH_SELECT_CLONAL_FAMILIES";
export const DETAIL_SELECT_CLONAL_FAMILIES = "DETAIL_SELECT_CLONAL_FAMILIES";
export const SELECT_TREE_NODES = "SELECT_TREE_NODES";
// Note that pagination includes sorting
export const UPDATE_DATASET_PAGINATION = "UPDATE_DATASET_PAGINATION";
export const UPDATE_CLONAL_FAMILY_PAGINATION = "UPDATE_CLONAL_FAMILY_PAGINATION";

export const DATASETS_RECEIVED = "DATASETS_RECEIVED";
export const CLONAL_FAMILIES_RECEIVED = "CLONAL_FAMILIES_RECEIVED";
export const TREE_RECEIVED = "TREE_RECEIVED";
export const TREE_ERROR = "TREE_ERROR";

export const PAGE_CHANGE = "PAGE_CHANGE";
export const URL = "URL";

// uncertain
export const UPDATE_TIP_RADII = "UPDATE_TIP_RADII";
export const NEW_COLORS = "NEW_COLORS";
export const FREQUENCY_MATRIX = "FREQUENCY_MATRIX";
export const BROWSER_DIMENSIONS = "BROWSER_DIMENSIONS";
export const BRANCH_MOUSEENTER = "BRANCH_MOUSEENTER";
export const BRANCH_MOUSELEAVE = "BRANCH_MOUSELEAVE";
export const NODE_MOUSEENTER = "NODE_MOUSEENTER";
export const NODE_MOUSELEAVE = "NODE_MOUSELEAVE";
export const SEARCH_INPUT_CHANGE = "SEARCH_INPUT_CHANGE";
export const CHANGE_LAYOUT = "CHANGE_LAYOUT";
export const CHANGE_BRANCH_LABEL = "CHANGE_BRANCH_LABEL";
export const CHANGE_DISTANCE_MEASURE = "CHANGE_DISTANCE_MEASURE";
export const CHANGE_DATES_VISIBILITY_THICKNESS = "CHANGE_DATES_VISIBILITY_THICKNESS";
export const CHANGE_ABSOLUTE_DATE_MIN = "CHANGE_ABSOLUTE_DATE_MIN";
export const CHANGE_ABSOLUTE_DATE_MAX = "CHANGE_ABSOLUTE_DATE_MAX";
export const CHANGE_GEO_RESOLUTION = "CHANGE_GEO_RESOLUTION";
export const APPLY_FILTER = "APPLY_FILTER";
export const UPDATE_VISIBILITY_AND_BRANCH_THICKNESS = "UPDATE_VISIBILITY_AND_BRANCH_THICKNESS";
export const TOGGLE_MUT_TYPE = "TOGGLE_MUT_TYPE";
export const CHANGE_ANALYSIS_VALUE = "CHANGE_ANALYSIS_VALUE";
export const CHANGE_ANIMATION_START = "CHANGE_ANIMATION_START";
export const CHANGE_ANIMATION_TIME = "CHANGE_ANIMATION_TIME";
export const CHANGE_ANIMATION_CUMULATIVE = "CHANGE_ANIMATION_CUMULATIVE";
export const CHANGE_ANIMATION_LOOP = "CHANGE_ANIMATION_LOOP";
export const TOGGLE_TEMPORAL_CONF = "TOGGLE_TEMPORAL_CONF";
export const MAP_ANIMATION_PLAY_PAUSE_BUTTON = "MAP_ANIMATION_PLAY_PAUSE_BUTTON";
export const ADD_NOTIFICATION = "ADD_NOTIFICATION";
export const REMOVE_NOTIFICATION = "REMOVE_NOTIFICATION";
export const CHANGE_PANEL_LAYOUT = "CHANGE_PANEL_LAYOUT";
export const TOGGLE_PANEL_DISPLAY = "TOGGLE_PANEL_DISPLAY";
export const TRIGGER_DOWNLOAD_MODAL = "TRIGGER_DOWNLOAD_MODAL";
export const DISMISS_DOWNLOAD_MODAL = "DISMISS_DOWNLOAD_MODAL";
export const ENTROPY_DATA = "ENTROPY_DATA";
export const ENTROPY_COUNTS_TOGGLE = "ENTROPY_COUNTS_TOGGLE";
export const CHANGE_URL_QUERY_BUT_NOT_REDUX_STATE = "CHANGE_URL_QUERY_BUT_NOT_REDUX_STATE";
export const MIDDLEWARE_ONLY_ANIMATION_STARTED = "MIDDLEWARE_ONLY_ANIMATION_STARTED";
export const URL_QUERY_CHANGE_WITH_COMPUTED_STATE = "URL_QUERY_CHANGE_WITH_COMPUTED_STATE";
export const PAGE_DOWN = "PAGE_DOWN";
export const PAGE_UP = "PAGE_UP";
export const TOGGLE_SORT = "TOGGLE_SORT";
export const UPDATE_BRUSH_SELECTION = "UPDATE_BRUSH_SELECTION";
export const TOGGLE_FAMILY = "TOGGLE_FAMILY";
export const UPDATE_SELECTED_SEQ = "UPDATE_SELECTED_SEQ";
export const UPDATE_SELECTED_TREE = "UPDATE_SELECTED_TREE";
export const SELECTING_STATUS = "SELECTING_STATUS";
export const UPDATE_FACET = "UPDATE_FACET";

export const RESET_CLONAL_FAMILIES_STATE = "RESET_CLONAL_FAMILIES_STATE";
export const LOADING_DATASET = "LOADING_DATASET";
export const FILTER_BRUSH_SELECTION = "FILTER_BRUSH_SELECTION";
export const CLEAR_BRUSH_SELECTION = "CLEAR_BRUSH_SELECTION";
export const FILTER_LOCUS = "FILTER_LOCUS";

// High-level filtering
export const SET_FILTER = "SET_FILTER";
export const CLEAR_FILTER = "CLEAR_FILTER";
export const CLEAR_ALL_FILTERS = "CLEAR_ALL_FILTERS";

export const REMOVE_DATASET = "REMOVE_DATASET";
export const DATASETS_UPDATED = "DATASETS_UPDATED";
export const TOGGLE_DATASET_SELECTION = "TOGGLE_DATASET_SELECTION";
export const CLEAR_DATASET_SELECTIONS = "CLEAR_DATASET_SELECTIONS";
export const BATCH_UPDATE_DATASETS = "BATCH_UPDATE_DATASETS";
export const CLEAR_PENDING_DATASET_LOADS = "CLEAR_PENDING_DATASET_LOADS";
export const SET_PENDING_DATASET_LOADS = "SET_PENDING_DATASET_LOADS";
export const UPDATE_SELECTED_CHAIN = "UPDATE_SELECTED_CHAIN";
export const UPDATE_LAST_CLICKED_CHAIN = "UPDATE_LAST_CLICKED_CHAIN";

// Lineage (ancestral sequence) settings
export const UPDATE_LINEAGE_SHOW_ENTIRE = "UPDATE_LINEAGE_SHOW_ENTIRE";
export const UPDATE_LINEAGE_SHOW_BORDERS = "UPDATE_LINEAGE_SHOW_BORDERS";
export const UPDATE_LINEAGE_SHOW_MUTATION_LABELS = "UPDATE_LINEAGE_SHOW_MUTATION_LABELS";
export const UPDATE_LINEAGE_CHAIN = "UPDATE_LINEAGE_CHAIN";

// Subtree focus state (shared between Phylogeny and Ancestral Sequences views)
export const UPDATE_SUBTREE_ROOT = "UPDATE_SUBTREE_ROOT";
export const UPDATE_TREAT_SUBTREE_AS_ROOT = "UPDATE_TREAT_SUBTREE_AS_ROOT";

// Visualization config actions
export const CONFIGS_LOADED = "CONFIGS_LOADED";
export const CONFIG_SAVED = "CONFIG_SAVED";
export const CONFIG_DELETED = "CONFIG_DELETED";
export const CONFIG_APPLIED = "CONFIG_APPLIED";
export const CONFIG_MODAL_OPEN = "CONFIG_MODAL_OPEN";
export const CONFIG_MODAL_CLOSE = "CONFIG_MODAL_CLOSE";

// UI state
export const UPDATE_CURRENT_SECTION = "UPDATE_CURRENT_SECTION";

// Table column layout (visibility / order)
export const SET_FAMILIES_COLUMN_VISIBILITY = "SET_FAMILIES_COLUMN_VISIBILITY";
export const SET_FAMILIES_COLUMN_VISIBILITY_MAP = "SET_FAMILIES_COLUMN_VISIBILITY_MAP";
export const SET_FAMILIES_COLUMN_ORDER = "SET_FAMILIES_COLUMN_ORDER";

// Starred families
export const TOGGLE_STARRED_FAMILY = "TOGGLE_STARRED_FAMILY";
export const CLEAR_STARRED_FAMILIES = "CLEAR_STARRED_FAMILIES";
export const SET_STARRED_FAMILIES = "SET_STARRED_FAMILIES";

// Starred datasets
export const TOGGLE_STARRED_DATASET = "TOGGLE_STARRED_DATASET";
export const CLEAR_STARRED_DATASETS = "CLEAR_STARRED_DATASETS";
export const SET_STARRED_DATASETS = "SET_STARRED_DATASETS";
