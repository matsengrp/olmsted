...
cft_to_olmsted_fns = dict(
    (key, remap_list) for key in ["datasets", "seeds", "subjects", "samples", "trees"]
)
...
cft_to_olmsted_mappings = {
    ...
    "datasets": {"id": "dataset_id", "clonal_families": "clones"},
}
...
def clean_record(d):
    if isinstance(d, list):
        ...
        return {trim_tripl_naming(k): clean_record(v) for k, v in d.items()}
...
subject_pull_pattern = ["db:ident", "cft.subject:id"]

seed_pull_pattern = ["db:ident", "cft.seed:id"]
...
reconstruction_pull_pattern = [
    ...
    "cft.reconstruction:prune_count",
    {
        ...
        "cft.reconstruction:asr_seqs": [
            "tripl.file:path",
            {"bio.seq:set": ["bio.seq:id", "bio.seq:seq"]},
        ],
    },
]
...
clonal_family_pull_pattern = [
    ...
    "cft.cluster:mean_mut_freq",
    {
        ...
        "cft.cluster:v_per_gene_support": [
            "db:ident",
            "cft.gene_support:gene",
            "cft.gene_support:prob",
        ],
        "cft.cluster:d_per_gene_support": [
            "db:ident",
            "cft.gene_support:gene",
            "cft.gene_support:prob",
        ],
        "cft.cluster:j_per_gene_support": [
            "db:ident",
            "cft.gene_support:gene",
            "cft.gene_support:prob",
        ],
...
def parse_tree_data(args, c):
    ...
    def process_node(node):
        ...
        for attr, parser in [
            ...
            try:
                value = (
                    (parser or (lambda x: x))(seqmeta.get(attr))
                    if seqmeta.get(attr)
                    else None
                )
        ...
        return {
            ...
            "timepoint_multiplicities": [
                {"timepoint_id": t, "multiplicity": m}
                for t, m in zip(
                    node.timepoints or [], node.timepoint_multiplicities or []
                )
            ],
...
def clean_reconstruction_record(args, d):
    c = d.copy()
    c["cft.reconstruction:downsampling_strategy"] = c[
        "cft.reconstruction:prune_strategy"
    ]
    ...
    for var in [
        "cft.reconstruction:cluster_aa",
        "cft.reconstruction:asr_seqs",
        "cft.reconstruction:seqmeta",
        "cft.reconstruction:prune_strategy",
        "cft.reconstruction:prune_count",
        "cft.reconstruction:asr_tree",
    ]:
...
def clean_clonal_family_record(args, d):
    try:
        c = d.copy()
        c["cft.cluster:trees"] = map(
            fun.partial(clean_reconstruction_record, args),
            c["cft.reconstruction:_cluster"],
        )
        for attr_key, attr_id_key in [
            ("cft.cluster:seed", "cft.seed:id"),
            ("cft.cluster:dataset", "cft.dataset:id"),
            ("cft.cluster:sample", "cft.sample:id"),
            ("cft.cluster:subject", "cft.subject:id"),
...
def pull_clonal_families(args, t):
    result = map(
        comp(clean_record, fun.partial(clean_clonal_family_record, args)),
    ...
    if bad_families:
        warnings.warn(
            "{} (of {}) clonal families couldn't be processed".format(
                len(bad_families), len(result)
            )
        )
    else:
...
def main():
        ...
        try:
            if args.data_outdir:
                ...
                assert len(file_datasets) == 1, (
                    "Must have exactly one dataset per file (for now); instead had %s"
                    % len(file_datasets)
                )
                ...
                remap_data_in_place(
                    {"datasets": [full_schema_dataset]}, cft_to_olmsted_mappings
                )
    ...
    if args.data_outdir:
        for d in full_schema_datasets:
            write_out(
                d, args.data_outdir, d["dataset_id"] + ".full_schema_dataset.json", args
            )
        ...
        for dataset_id, clonal_families in clonal_families_dict.items():
            write_out(
                clonal_families,
                args.data_outdir + "/",
                "clonal_families." + dataset_id + ".json",
                args,
            )
