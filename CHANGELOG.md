...
* The clonal-families table's optional columns can be **reordered by drag-and-drop** (#323), via @dnd-kit — drag a column header's grip, or use the keyboard (focus the grip, Space to pick up, arrows to move, Space to drop). Required columns stay fixed at the front. The order persists per browser. Column resize now composes with reorder: widths are keyed by column identity rather than position, so a resized column keeps its width when moved or when other columns are hidden.
* The **in-app dataset loading table** and **splash dataset management table** now have the same Columns picker + drag-and-drop reordering (#323). Loading table: Star/Select/Status/Info/Name are required; Source/Size/Subjects/Families/Upload Time/Build Time/Missing Fields are optional. Management table: Star/Load/Info/Name/Delete are required (Delete stays pinned to the end); the same set of fields is optional. All three tables' column-layout state is now backed by one generic Redux slice (`tableColumns`) and one shared picker/reorder/header implementation.
* Table column layout (visibility + order, for all three tables) is now part of the **visualization config / user-settings system** (#323): saving a named config captures the current layout, applying a config restores it, and the layout persists per browser via `sessionStorage` independent of saved configs. `CONFIG_VERSION` bumped 1.2 → 1.3; configs saved before 1.3 (which lack table settings) continue to load without error and leave the current layout untouched.
