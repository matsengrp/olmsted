...
5. **Browser smoke test**: With `npm start`, exercise the three flows end-to-end in a real browser. The Vega rendering layer is not exercised in headless tests beyond spec parse + headless View; visual regressions and signal-listener bugs only show up here.
   ...
   - **Automated coverage (Playwright):** A happy-path end-to-end test (`tests/e2e/smoke.spec.js`) exercises upload → scatterplot → brush → table → tree → subtree-focus and asserts on Vega View state. Run it with `npm run test:e2e` (see [DEVELOPMENT.md](./DEVELOPMENT.md#end-to-end-tests-playwright)); CI runs it on every push. If the PR only touches reducers, selectors, utilities, or other non-render code and `npm run test:e2e` passes, the manual walk-through above can be skipped — note "browser smoke test skipped — Playwright passes" in the PR body. Visualization changes (Vega specs, chart components, splash flow) still require a manual pass: Phase 1 catches data-wiring regressions, not visual ones.
...
17. **GitHub auto-linking**: In PR/issue text and comments, don't prefix bare numbers with `#` (e.g. write "finding 3", not "#3") — GitHub auto-links `#N` to whatever issue/PR has that number, creating misleading links. Naked numbers are fine; reserve `#N` for genuine cross-references (e.g., "closes #286").
