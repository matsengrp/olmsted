...
class PaginationControls extends React.Component {
  render() {
    const is_first_page = this.props.pagination.page == 0;
    const is_last_page = this.props.pagination.page == this.props.last_page;
    return (
...
class Table extends React.Component {
  render() {
    const nCols = this.props.mappings.length;
    const templateColumnsStyle = "auto ".repeat(nCols);
    return (
      ...
      >
        ...
        { _.map(this.props.mappings, ([name, AttrOrComponent]) => {
          ...
          const is_sorting = this.props.pagination.order_by == AttrOrComponent;
          if (is_sorting) {
            const sorting_arrow = this.props.pagination.desc ? ' \u25BC' : ' \u25B2';
            name += sorting_arrow;
          }
          const style = is_sorting ? {fontSize: 14, fontWeight: "bold"} : {fontSize: 13};
        ...
        {this.props.data.map((datum) => {
          return _.map(this.props.mappings, ([name, AttrOrComponent]) => {
            ...
            return (
              <div className="grid-item" key={key} style={style}>
                {isAttr
...
const mapStateToProps = (state) => {
  const newClonalFamiliesPage = getClonalFamiliesPage(state);
  return {
