...
class SplitFileProcessor {
    ...
    static async processFiles(files) {
        try {
            ...
            const processedResults = datasets.map(dataset => {
                ...
                const datasetClones = clones.filter(clone => 
                ...
                const datasetTrees = trees.filter(tree =>
                    tree.clone_id && datasetClones.some(clone => 
                        clone.clone_id === tree.clone_id || 
    ...
    static isSplitFormat(fileContents) {
        ...
        const hasDatasetContent = fileContents.some(f => 
