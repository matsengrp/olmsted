  ...
  _.forEach(tree, (node) => {
    ...
    _.forEach(pairs, (pair) => {
      ...
      if (aa != naive_seq[i]) {
        ...
        mutations.push({
          type: node.type, parent: node.parent, seq_id: seq_id, position: i, mut_from: naive_seq[i], mut_to: aa
        });
      } else if (is_naive) {
        // add a mutation for the naive so it shows up in the viz
        mutations.push({
          type: 'naive', parent: node.parent, seq_id: seq_id, position: i, mut_from: naive_seq[i], mut_to: aa
        });
      }
    });
...
const uniqueSeqs = (nodes) => {
  ...
  const naive = _.remove(seq_records, (o) => { return o.type == "root"; })[0];
  const leaves = _.remove(seq_records, (o) => { return o.type == "leaf"; });
  ...
  let download_seqs = [];
  const uniq_int_nodes = _.filter(
    _.uniqBy(
      _.reverse(internal_nodes),
      'sequence_alignment'
    ),
    (node) => {return !taken_seqs.has(node.sequence_alignment);}
  );
