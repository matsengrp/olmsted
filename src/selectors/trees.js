...
export const getSelectedSeq = createSelector(
  ...
  (seq_id, tree) => _.find(tree.nodes, {"sequence_id": seq_id})
)
...
const createAlignment = (naive_seq, tree) => {
  ...
  _.forEach(tree, (node) => {
    let mutations = []
    let seq = node.sequence_alignment_aa;
    let seq_id = node.sequence_id;
...
const followLineage = (nodes, leaf, naive) => {
  ...
  let taken_seqs = new Set([leaf.sequence_alignment, naive.sequence_alignment])
  ...
  while (curr_node.parent){
    ...
    let parent = _.find(nodes, {"sequence_id": parent_id});
...
const uniqueSeqs = (nodes) => {
  ...
  let taken_seqs = new Set([_.map(leaves, (leaf) => {leaf.sequence_alignment}).concat([naive.sequence_alignment])])
  var download_seqs = [];
  let uniq_int_nodes = _.filter(
                          _.uniqBy(
                              _.reverse(internal_nodes),
                              'sequence_alignment'),
                          function(node) {return !taken_seqs.has(node.sequence_alignment)})
...
const computeLineageData = (tree, seq) => {
  ...
  if (tree["nodes"] && tree["nodes"].length > 0 && !_.isEmpty(seq)){
    ...
    let alignment = createAlignment(naive.sequence_alignment_aa, lineage)
