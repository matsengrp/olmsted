...
const initialState = {
  ...
  byIdent: {},
  pagination: {
    page: 0, per_page: 10, order_by: "unique_seqs_count", desc: true
  },
...
const clonalFamilies = (state = _.clone(initialState), action) => {
  switch (action.type) {
    case types.RESET_CLONAL_FAMILIES_STATE: {
      ...
      const updatedByIdent = Object.assign({}, state.byIdent, _.fromPairs(_.map(action.clonalFamilies, (x) => [x.ident, x])));
